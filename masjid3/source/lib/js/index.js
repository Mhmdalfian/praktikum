document.getElementById("btn-toggle").addEventListener("click", () => {
  ...
  $(document).ready(function(){
    ...
    $(document).on("click",".btn-edit-petugas",function(){
        ...
        let record = table.row(posisiBaris).data();
        $('#petugasModal').modal('show');
        $('#btnPetugas').text('Ubah');
        $('#titlePetugasModal').text('Ubah data');
        $('#txtnik').val(record.nik);
        $('#txtnama').val(record.nama);
        $('#txttempat').val(record.tempat_lahir);
        $('#txttgl').val(record.tanggal_lahir);
        $('#txttelp').val(record.telp);
        $('#txtalamat').val(record.alamat);
      });
